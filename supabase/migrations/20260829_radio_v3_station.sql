...
-- ============================================================
-- Extends the radio V1 / V2 schema. Renumbered from 20260827 to 20260829
-- so it lands AFTER 20260826_rls_admin_hardening.sql (profiles, is_admin,
-- is_radio_staff) and 20260828_tft_permissions_rls.sql (the six-level role
-- model). Those two own authorisation; this file builds on them rather
-- than re-deciding it. Requires 20260824_core_tables_events_radio_shows.sql
-- for the radio_shows and events foreign keys.
--
-- DESIGN NOTES
...
-- RECONCILED — delegates to the authoritative role model.
--
-- V3 originally defined this against auth.users.raw_user_meta_data->>'role'.
-- That is a SECOND role model competing with the `profiles`-based one
-- established in 20260826_rls_admin_hardening.sql and extended by
-- 20260828_tft_permissions_rls.sql. Two sources of truth for "is this
-- user staff" is how privilege drift starts: granting someone a role in
-- `profiles` would not have granted it here, and vice versa.
--
-- The name is kept so V3's ~20 policies below need no rewriting, but the
-- body now defers entirely to public.is_radio_staff(). There is exactly
-- one place where radio staff membership is decided.
...
--     page fails with "permission denied for table users".
--
--     RECONCILED: 20260828_tft_permissions_rls.sql already replaced the
--     WRITE side of these six tables with a finer contributor/manager
--     model (contributor_add_media, radio_manager_manage_sponsors, ...)
--     -- but it drops its own policy names, never V1's, so V1's broken
--     policies are still live. Only the DROPs are kept below. V3's
--     original blanket `for all using (radio_is_staff())` replacements
--     were REMOVED: re-adding them would OR with 20260828's policies and
--     silently widen every contributor to full staff write access.
--     20260828 already covers writes on all six tables.
--
...
-- but section 4 above redefines `status` to mean HOW a programme is
-- broadcast, constrained to ('planned','live','pre-recorded','automated').
-- 'scheduled' and 'archived' are no longer reachable values, so that
-- policy would publish every 'live' programme regardless of its
-- publication state, ORed with the content_status rule below. Dropped.
...
drop policy if exists "radio_sponsors_public_read" on radio_sponsors;
...
drop policy if exists "radio_media_public_read" on radio_media;
...
-- RECONCILED — 20260828_tft_permissions_rls.sql owns the storage policies.
--
-- V3 originally created four blanket policies here
--   (radio_audio_staff_write / _staff_update / _staff_delete, each
--    `bucket_id in ('radio-audio','radio-images') and radio_is_staff()`).
-- 20260828 already policies the radio-audio bucket on a tighter,
-- owner-scoped model: uploads require `owner = auth.uid()`, and update
-- and delete require `owner = auth.uid() or is_radio_manager()`. Since
-- policies are permissive and ORed, adding V3's versions would have let
-- any radio staff member overwrite or delete another member's audio.
--
-- What V3 genuinely adds is the second bucket, radio-images. So instead
-- of replacing 20260828's policies, they are extended to cover it,
-- keeping the owner-scoped rules intact.
...
drop policy if exists "radio_images_staff_upload" on storage.objects;
create policy "radio_images_staff_upload" on storage.objects
  for insert to authenticated
  with check (
    bucket_id = 'radio-images'
    and public.is_radio_staff()
    and owner = auth.uid()
  );

drop policy if exists "radio_images_owner_update" on storage.objects;
create policy "radio_images_owner_update" on storage.objects
  for update to authenticated
  using       (bucket_id = 'radio-images'
               and (owner = auth.uid() or public.is_radio_manager()))
  with check  (bucket_id = 'radio-images'
               and (owner = auth.uid() or public.is_radio_manager()));

drop policy if exists "radio_images_owner_delete" on storage.objects;
create policy "radio_images_owner_delete" on storage.objects
