...
interface CommunityEvent {
  ...
  date: string;
  location: string;
  category: 'workshop' | 'radio-live' | 'community' | 'volunteer';
  organiser: string;
}
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-6">
      <div className="flex flex-col space-y-2 md:flex-row md:justify-between md:items-center">
        ...
        <Button onClick={() => { window.location.href = '/submit-listing'; }}>Host an Event</Button>
      </div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline">{selectedDate ? selectedDate.toLocaleDateString() : 'Pick a date'}</Button>
          </PopoverTrigger>
      ...
      <div className="flex flex-wrap gap-2">
        {['all', 'workshop', 'radio-live', 'community', 'volunteer'].map((cat) => (
          <Badge key={cat} variant={activeCategory === cat ? 'default' : 'secondary'} className="cursor-pointer capitalize px-3 py-1 text-sm" onClick={() => setActiveCategory(cat)}>
            {cat.replace('-', ' ')}
          </Badge>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <Card key={i} className="w-full">
              <CardHeader className="space-y-2"><Skeleton className="h-5 w-2/3" /><Skeleton className="h-4 w-1/2" /></CardHeader>
            ...
            <Card key={event.id} className="flex flex-col justify-between hover:shadow-md transition-shadow">
              <CardHeader>
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-xs text-muted-foreground">{new Date(event.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                </div>
                <CardTitle className="line-clamp-1">{event.title}</CardTitle>
                <CardDescription>{event.location}</CardDescription>
              </CardHeader>
              <CardContent><p className="text-sm text-muted-foreground line-clamp-3">{event.description}</p></CardContent>
              <CardFooter className="pt-0 flex justify-between items-center text-xs text-muted-foreground border-t mt-4 p-4">
                <span>By {event.organiser}</span>
                <Button size="sm" variant="ghost">View Details</Button>
              </CardFooter>
            </Card>
          )) : <div className="col-span-full text-center py-12 text-muted-foreground">No events found matching your criteria.</div>}
        </div>
      )}
    </div>
  );
