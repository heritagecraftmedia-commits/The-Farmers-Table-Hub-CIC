...
export const RadioStudioDashboard: React.FC = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [m, p] = await Promise.all([radioService.getActiveMedia(), radioService.getReadyPlaylists()]);
      setMedia(m);
      setPlaylists(p);
      setStatus('');
    } catch (e) {
      console.error(e);
      setStatus('Connect the Radio V1 Supabase migration to use the live library.');
  ...
  const add = (item: RadioMedia, next = false) => {
    const entry = { ...item, queueId: `${item.id}-${Date.now()}-${Math.random()}` };
    setQueue(current => next ? [entry, ...current] : [...current, entry]);
  };
  ...
  const loadPlaylist = async (playlist: RadioPlaylist) => {
    try {
      ...
      console.error(e);
      setStatus('This playlist has no readable items yet.');
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
        <div>
          <div className="flex items-center gap-2 text-brand-olive text-xs font-bold uppercase tracking-widest"><Radio size={15} /> Studio Dashboard</div>
          <h2 className="text-3xl md:text-4xl font-serif mt-2">Build today's radio slot</h2>
          <p className="text-brand-ink/50 mt-2">Prepare the content here. Live365 remains the broadcast and outside-broadcast service.</p>
        </div>
        <div className="flex items-center gap-2 bg-white border border-brand-olive/10 rounded-2xl px-4 py-3">
          <Clock3 size={17} className="text-brand-olive" />
          <label className="text-xs font-bold">Slot</label>
          <select value={slotMinutes} onChange={e => setSlotMinutes(Number(e.target.value))} className="bg-transparent font-bold outline-none">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.15fr_.85fr] gap-8">
        <section className="bg-white rounded-[32px] border border-brand-olive/5 p-6 md:p-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div><h3 className="text-2xl font-serif">Library</h3><p className="text-sm text-brand-ink/50">One button adds an item. Add Next puts a request at the front.</p></div>
            <div className="relative"><Search size={16} className="absolute left-3 top-3 text-brand-ink/30" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search music, jingles..." className="pl-9 pr-4 py-2.5 rounded-xl bg-brand-cream outline-none text-sm w-full md:w-64" /></div>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-2 mb-4">
            {['all', 'music', 'jingle', 'community', 'ad'].map(c => <button key={c} onClick={() => setCategory(c)} className={`px-4 py-2 rounded-full text-xs font-bold capitalize whitespace-nowrap ${category === c ? 'bg-brand-olive text-white' : 'bg-brand-cream text-brand-ink/60'}`}>{c}</button>)}
          </div>
          {loading ? <p className="text-sm text-brand-ink/50">Loading real audio library…</p> : filtered.length === 0 ? <p className="text-sm text-brand-ink/50 py-8">No real audio matches yet. Add music, jingles, community notices or adverts to the library.</p> : <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
            {filtered.map(item => <div key={item.id} className="rounded-2xl bg-brand-cream p-4 flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-brand-olive/10 flex items-center justify-center shrink-0"><Volume2 size={17} className="text-brand-olive" /></div>
              <div className="min-w-0 flex-1"><p className="font-bold truncate">{item.title}</p><p className="text-xs text-brand-ink/50 truncate">{item.artist || item.category} · {formatTime(item.durationSeconds)}</p></div>
              <button onClick={() => add(item)} title="Add to slot" className="p-2.5 rounded-xl bg-white text-brand-olive hover:bg-brand-olive hover:text-white transition"><Plus size={18} /></button>
              <button onClick={() => add(item, true)} title="Add next" className="px-3 py-2.5 rounded-xl bg-white text-brand-ink text-xs font-bold hover:bg-brand-olive hover:text-white transition">Add Next</button>
            </div>)}
          </div>}
        </section>

        <section className="bg-brand-ink text-brand-cream rounded-[32px] p-6 md:p-8">
          <div className="flex items-start justify-between gap-4 mb-6"><div><h3 className="text-2xl font-serif">Today's slot</h3><p className="text-sm text-brand-cream/50">Prepared queue</p></div><div className="text-right"><p className="text-2xl font-bold">{formatTime(used)}</p><p className="text-xs text-brand-cream/50">{formatTime(remaining)} remaining</p></div></div>
          <div className="h-2 bg-white/10 rounded-full overflow-hidden mb-6"><div className="h-full bg-brand-cream rounded-full" style={{ width: `${Math.min((used / (slotMinutes * 60)) * 100, 100)}%` }} /></div>
