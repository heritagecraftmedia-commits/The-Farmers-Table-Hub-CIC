...
import { ChevronDown, ChevronUp, Clock3, ListMusic, Plus, Radio, Search, SkipForward, Trash2, Volume2, WandSparkles } from 'lucide-react';
...
export const RadioStudioDashboard: React.FC = () => {
  ...
  const categoryCounts = queue.reduce<Record<string, number>>((acc, item) => { acc[item.category] = (acc[item.category] || 0) + 1; return acc; }, {});
  ...
  const buildProgramme = (programme: typeof PROGRAMMES[number]) => {
    const available = [...media];
    const chosen: RadioMedia[] = [];
    const lastIds = new Set<string>();
    for (const type of programme.pattern) {
      const candidates = available.filter(item => item.category === type && !lastIds.has(item.id));
      if (!candidates.length) continue;
      const item = candidates[Math.floor(Math.random() * candidates.length)];
      chosen.push(item);
      lastIds.add(item.id);
    }
    if (!chosen.length) return setStatus('There is not enough audio in the library yet. Add music, jingles, adverts and community items first.');
    setSlotMinutes(programme.minutes);
    setQueue(chosen.map((item, i) => ({ ...item, queueId: `${item.id}-${Date.now()}-${i}` })));
    setPlaylistName(`Farmers Table Radio — ${programme.name}`);
    const missing = programme.pattern.filter(type => !media.some(item => item.category === type)).filter((v, i, a) => a.indexOf(v) === i);
    setStatus(missing.length ? `${programme.name} built from what is available. Still needed in the library: ${missing.join(', ')}.` : `${programme.name} built with a varied mix. Check the order, then Save ready.`);
  };
