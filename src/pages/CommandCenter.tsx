...
export const CommandCenter: React.FC = () => {
    ...
    const tabButton = (id: CentralTab, label: string) => (
        <button onClick={() => setActiveTab(id)} className={`px-4 py-2 rounded-full text-sm font-bold transition-all whitespace-nowrap ${activeTab === id ? 'bg-brand-olive text-white shadow-md shadow-brand-olive/10' : 'text-brand-ink/50 hover:bg-white/50 hover:text-brand-olive'}`}>
            {label}
        </button>
    );
        ...
        <div className="min-h-screen bg-brand-cream pb-20">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            <div className="flex items-center gap-3 mb-2">
                                <span className="text-sm font-bold text-brand-olive bg-brand-olive/10 px-3 py-1 rounded-full">Central Command</span>
                                <div className="flex gap-2">{statusChip('Xero', 'live')}{statusChip('Notion', 'live')}{statusChip('HubSpot', 'pending')}{statusChip('Radio FM', 'live')}</div>
                            </div>
                            <h1 className="text-4xl md:text-5xl font-serif">{getGreeting()}, <span className="italic text-brand-olive">{user.name.split(' ')[0]}</span></h1>
                            <p className="text-brand-ink/50 mt-2 flex items-center gap-2"><Clock size={14} /> {currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} · Central Command — everything in one place</p>
                        </div>
                    </div>

                    <div className="flex gap-2 mt-8 overflow-x-auto pb-2 scrollbar-hide">
                        {tabButton('overview', 'Overview')}{tabButton('safemode', 'Safe Mode')}{tabButton('people', 'People')}{tabButton('tasks', 'Tasks')}{tabButton('radio', 'Radio')}{tabButton('stock', 'Café Stock')}{tabButton('advertisers', 'Advertisers')}{tabButton('finance', 'Finance')}{tabButton('schedules', 'Rotas')}{tabButton('events', 'Events')}{tabButton('records', 'Records')}
                        <button onClick={() => navigate('/discovery')} className="px-4 py-2 rounded-full text-sm font-bold transition-all whitespace-nowrap text-brand-ink/50 hover:bg-white/50 hover:text-brand-olive">Discovery</button>
                    </div>
                </div>
            </div>
            ...
            <main className="max-w-7xl mx-auto px-4 pt-10">
                <AnimatePresence mode="wait"><motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
                    ...
                    {activeTab === 'safemode' && <CentralSafeMode />}{activeTab === 'people' && <CentralPeople />}{activeTab === 'tasks' && <CentralTasks />}{activeTab === 'radio' && <CentralRadio />}{activeTab === 'stock' && <CentralStock />}{activeTab === 'advertisers' && <CentralAdvertisers />}{activeTab === 'finance' && <CentralFinance />}{activeTab === 'schedules' && <CentralSchedules />}{activeTab === 'events' && <CentralEvents />}{activeTab === 'records' && <CentralRecords />}
