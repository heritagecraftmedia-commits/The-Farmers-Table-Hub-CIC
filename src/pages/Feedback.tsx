...
import { MessageSquare, ThumbsUp, ThumbsDown, Send, CheckCircle, Printer, X } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { supabase } from '../lib/supabase';

type FeedbackForm = {
  name: string; email: string; what_works: string; what_doesnt_work: string; whats_missing: string;
  would_use: string; who_benefits: string; support_condition: string; money_thoughts: string;
  community_impact: string; change_one_thing: string; add_one_thing: string; anything_else: string;
};

type FieldProps = {
  label: string; value: string; field: keyof FeedbackForm; placeholder: string; rows?: number;
...
const initialForm: FeedbackForm = {
  name: '', email: '', what_works: '', what_doesnt_work: '', whats_missing: '', would_use: '',
  who_benefits: '', support_condition: '', money_thoughts: '', community_impact: '',
  change_one_thing: '', add_one_thing: '', anything_else: '',
};
...
export const Feedback: React.FC = () => {
  ...
  const update = (key: keyof FeedbackForm, value: string) => setForm(prev => ({ ...prev, [key]: value }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(false);
    if (saveError) {
      setError('We could not save this feedback online. Please use the email fallback below.');
      return;
    }
    setFormOpen(false);
  ...
  const fallbackSubject = encodeURIComponent('Farmers Table Feedback Book — Submission');
  const fallbackBody = encodeURIComponent(
    `Farmers Table Feedback Book response\n\nFirst impression: ${overall || 'Not selected'}\n\nWhat works?\n${form.what_works}\n\nWhat doesn't work?\n${form.what_doesnt_work}\n\nWhat's missing?\n${form.whats_missing}\n\nWould you use it? Why?\n${form.would_use}\n\nWho benefits most?\n${form.who_benefits}\n\nWhat would make you support it?\n${form.support_condition}\n\nMoney / sustainability thoughts?\n${form.money_thoughts}\n\nCommunity impact?\n${form.community_impact}\n\nChange one thing?\n${form.change_one_thing}\n\nAdd one thing?\n${form.add_one_thing}\n\nAnything else?\n${form.anything_else}\n\nName: ${form.name}\nEmail: ${form.email}`
  );
    ...
    <div className="py-10 md:py-16 bg-brand-cream min-h-screen print:bg-white print:py-0">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 print:mb-6">
          <h1 className="text-5xl md:text-7xl font-serif mb-5 mt-6">Tell us what you <span className="italic text-brand-olive">really think.</span></h1>
        ...
        <AnimatePresence>
          {formOpen && (
            <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="feedback-dialog-title"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={e => { if (e.target === e.currentTarget) closeForm(); }}>
              <motion.div className="relative w-full max-w-4xl max-h-[92vh] overflow-y-auto bg-white rounded-[32px] shadow-2xl p-7 md:p-12"
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      {([['Great', ThumbsUp], ['Suggestions', MessageSquare], ['Concerns', ThumbsDown]] as const).map(([label, Icon]) => (
                        <button key={label} type="button" onClick={() => setOverall(label)} className={`py-5 rounded-2xl flex flex-col items-center gap-2 border-2 transition-all ${overall === label ? 'border-red-600 bg-red-50' : 'border-brand-olive/10 bg-brand-cream/40 hover:border-brand-olive/30'}`}>
                          <Icon size={26} className="text-brand-olive" aria-hidden="true" /><span className="font-bold">{label}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2"><label className="text-sm font-bold uppercase tracking-widest opacity-70">Name (optional)</label><input value={form.name} onChange={e => update('name', e.target.value)} className="w-full p-4 rounded-2xl bg-brand-cream/50 border border-brand-olive/10" /></div>
                    <div className="space-y-2"><label className="text-sm font-bold uppercase tracking-widest opacity-70">Email (optional)</label><input type="email" value={form.email} onChange={e => update('email', e.target.value)} className="w-full p-4 rounded-2xl bg-brand-cream/50 border border-brand-olive/10" /></div>
                  </div>

                  {error && <div className="p-4 rounded-2xl bg-red-50 border border-red-200 text-red-800" role="alert"><p className="font-bold">{error}</p><a className="inline-flex mt-3 px-4 py-2 rounded-full bg-red-600 text-white font-bold hover:bg-red-700" href={`mailto:thefarmerstable.hub@gmail.com?subject=${fallbackSubject}&body=${fallbackBody}`}>Email the Feedback Book instead</a></div>}
        ...
        {messageReceived && <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[70] bg-red-600 text-white rounded-2xl shadow-2xl px-5 py-3 flex items-center gap-4 print:hidden" role="status" aria-live="polite"><div className="flex items-center gap-2 font-black"><CheckCircle size={20} aria-hidden="true" /> MESSAGE RECEIVED</div><button type="button" onClick={() => setMessageReceived(false)} className="px-3 py-1 rounded-full bg-white text-red-600 font-bold">Close me</button></div>}
